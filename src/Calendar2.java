...
public class Calendar2 extends javax.swing.JPanel { 
    ...
    public Calendar2() {
        initComponents();
        thisMonth();
        calendar = Calendar.getInstance(); // initialize calendar to current date
    ...
    private void initComponents() {
        ...
        jPanel2.setBackground(new java.awt.Color(97, 49, 237));
 ...
 public void setDayBackground(int day, Color color) {
  // الحصول على اليوم المحدد في التقويم
  Calendar calendar = Calendar.getInstance();
  calendar.set(Calendar.DAY_OF_MONTH, day);

  // الحصول على اليوم في التقويم الذي تعرضه JDayChooser
  Component[] dayComponents = this.getDayPanel().getComponents();

  for (Component c : dayComponents) {
      // تحقق من أن هذا اليوم هو الذي نريد تلوينه
      if (c instanceof javax.swing.JLabel) {
          javax.swing.JLabel label = (javax.swing.JLabel) c;
          int currentDay = Integer.parseInt(label.getText());

          // تحقق مما إذا كان اليوم الذي نريد تلوينه يطابق اليوم الحالي في الـ JDayChooser
          if (currentDay == day) {
              label.setBackground(color);  // تغيير اللون الخلفي
              label.setOpaque(true);        // جعل الخلفية مرئية
          }
      }
  }
}
